...
DATABASE_URL = os.environ.get('DATABASE_URL')  # напр. postgres://user:pass@host:5432/db
USE_PG = bool(DATABASE_URL and DATABASE_URL.startswith(('postgres://', 'postgresql://')))
SQLITE_PATH = os.path.join(BASE, 'hvh.db')

ADMIN_PASSWORD = os.environ.get('ADMIN_PASSWORD', '123wue123')
DEFAULT_ADMIN_EMAIL = os.environ.get('DEFAULT_ADMIN_EMAIL', 'dimacontrol2223@gmail.com')

app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', 'hvh-agressive-conference-secret-key-change-me')
app.config['MAX_CONTENT_LENGTH'] = 32 * 1024 * 1024

# ---------------------------------------------------------------------------
# DB layer: единый API для SQLite и Postgres.
# В коде используем плейсхолдер '?', слой сам подставит '%s' для Postgres.
# ---------------------------------------------------------------------------
if USE_PG:
    import psycopg
...
class _Cur:
    """Курсор с fetchone/fetchall, где строку можно читать и как dict, и как row['x']."""
    ...
    def __getitem__(self, k):
        # для случаев вида: c.execute(...).fetchone()['p']
        raise TypeError("use .fetchone()/.fetchall() first")
...
CREATE TABLE IF NOT EXISTS purchases(
    id SERIAL PRIMARY KEY,
    user_id INTEGER NOT NULL, config_id INTEGER NOT NULL,
    price INTEGER, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
CREATE TABLE IF NOT EXISTS reviews(
    id SERIAL PRIMARY KEY,
    user_id INTEGER, username TEXT, text TEXT NOT NULL,
    rating INTEGER DEFAULT 5, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
CREATE TABLE IF NOT EXISTS keys(
    id SERIAL PRIMARY KEY,
    code TEXT UNIQUE NOT NULL, amount INTEGER NOT NULL,
...
def _upsert_singleton(c, table, content):
    """Аналог INSERT OR IGNORE для строки с id=1."""
    row = c.execute(f"SELECT 1 FROM {table} WHERE id=1").fetchone()
    if not row:
        c.execute(f"INSERT INTO {table}(id, content) VALUES(1, ?)", (content,))
...
def init_db():
    ...
    _upsert_singleton(c, 'welcome',
        "Добро пожаловать в HvH Agressive Conference — комьюнити самых агрессивных HvH игроков. "
        "Здесь ты найдёшь топовых людей сцены, эксклюзивные конфиги и место, где твой скилл имеет значение.")
    _upsert_singleton(c, 'about',
        "HvH (Hack vs Hack) — противостояние читеров, где побеждает не рука, а мозг. "
        "HvH Agressive Conference — сообщество, где обсуждают конфиги, resolver'ы, anti-aim'ы и всё, "
        "что делает игру по-настоящему агрессивной.")

    # championship singleton
    if not c.execute("SELECT 1 FROM championship WHERE id=1").fetchone():
        c.execute(
            "INSERT INTO championship(id,title,date,description) VALUES(1,?,?,?)",
            ("HvH Agressive Championship #1", "",
             "Регистрация открыта. Нажми кнопку «Зарегистрироваться», чтобы попасть в сетку."))

    # default admin email
    if not c.execute("SELECT 1 FROM admin_emails WHERE lower(email)=lower(?)",
                     (DEFAULT_ADMIN_EMAIL,)).fetchone():
        try:
            c.execute("INSERT INTO admin_emails(email) VALUES(?)", (DEFAULT_ADMIN_EMAIL,))
        except IntegrityError:
            pass

    # sync is_admin для существующих пользователей
...
def championship():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            c.conn.rollback() if USE_PG else None
            flash('Ты уже зарегистрирован', 'err')
    ch = c.execute("SELECT * FROM championship WHERE id=1").fetchone()
    players = c.execute(
        "SELECT u.username, u.avatar, u.rating FROM championship_players p "
        "JOIN users u ON u.id = p.user_id ORDER BY u.rating DESC").fetchall()
    registered = False
    if u:
        registered = bool(c.execute("SELECT 1 FROM championship_players WHERE user_id=?",
                                    (u['id'],)).fetchone())
    c.close()
    pairs = [(players[i], players[i + 1] if i + 1 < len(players) else None)
...
def configs():
    c = db()
    items = c.execute(
        "SELECT c.*, u.username AS seller FROM configs c "
...
def buy(cid):
    ...
    c.execute("INSERT INTO purchases(user_id, config_id, price) VALUES(?,?,?)",
              (u['id'], cid, cfg['price']))
    c.commit(); c.close()
...
def download(cid):
    u = current_user()
    c = db()
    p = c.execute("SELECT * FROM purchases WHERE user_id=? AND config_id=?",
                  (u['id'], cid)).fetchone()
...
def cabinet():
    u = current_user()
    c = db()
    place = c.execute("SELECT COUNT(*) + 1 AS p FROM users WHERE rating > ?",
                      (u['rating'],)).fetchone()['p']
    top = c.execute("SELECT username, avatar, rating FROM users ORDER BY rating DESC LIMIT 5").fetchall()
    my_configs = c.execute("SELECT * FROM configs WHERE seller_id=? ORDER BY created_at DESC",
                           (u['id'],)).fetchall()
    purchased = c.execute(
        "SELECT c.* FROM purchases p JOIN configs c ON c.id = p.config_id "
...
def sell_config():
    ...
    c = db()
    c.execute("INSERT INTO configs(title, description, price, filename, seller_id, approved) "
              "VALUES(?,?,?,?,?,1)", (title, desc, price, fn, u['id']))
    c.commit(); c.close()
...
def admin_panel():
    ...
    configs = c.execute(
        "SELECT c.*, u.username AS seller FROM configs c "
        "LEFT JOIN users u ON u.id = c.seller_id ORDER BY c.created_at DESC").fetchall()
    reviews = c.execute("SELECT * FROM reviews ORDER BY created_at DESC").fetchall()
    keys = c.execute(
        "SELECT k.*, u.username AS user FROM keys k "
        "LEFT JOIN users u ON u.id = k.used_by ORDER BY k.id DESC").fetchall()
    ...
    ch = c.execute("SELECT * FROM championship WHERE id=1").fetchone()
    ch_players = c.execute(
        "SELECT p.id AS pid, u.username FROM championship_players p "
        "JOIN users u ON u.id = p.user_id ORDER BY u.rating DESC").fetchall()
...
def admin_championship():
    ...
    c.execute("UPDATE championship SET title=?, date=?, description=? WHERE id=1",
              (request.form.get('title', ''), request.form.get('date', ''),
...
def admin_user(uid):
    ...
    c = db()
    if action == 'rating':
        c.execute("UPDATE users SET rating=? WHERE id=?",
                  (int(request.form.get('rating', 0)), uid))
    elif action == 'balance':
        c.execute("UPDATE users SET balance=? WHERE id=?",
                  (int(request.form.get('balance', 0)), uid))
    elif action == 'avatar':
        f = request.files.get('avatar')
        if f and f.filename:
...
def admin_config_new():
    ...
    c = db()
    c.execute("INSERT INTO configs(title, description, price, filename, seller_id, approved) "
              "VALUES(?,?,?,?,NULL,1)", (title, desc, price, fn))
    c.commit(); c.close()
    return redirect(url_for('admin_panel'))
